...
int ReverseList(linklist H)
{
    ...
    /* 链表的头结点不存储数据为空 */
    p = H->pNext->pNext;    //p指向链表的第二个结点，防止后续置空后续节点丢失，其实此处p就相当于第二个结点的指针
    H->pNext->pNext = NULL; //链表的第二个结点置空，即头节点后面的那个结点，相当于此时H为空表

    while (p != NULL)
    {
        q = p;        //此处不能用节点p直接进行后续操作，使用节点q辅助后完成后续操作
        p = p->pNext;
...
int PurgeList(linklist H)
{
    ...
    while (p != NULL)
    {
        ...
        {
            h->pNext = p; //更新前一个结点的pNext指针
            h = p;        //更新前一个结点
        }

        p = p->pNext; //尾结点指针pNext指向NULL
    }
